#!/usr/bin/env python
# A simple Python script to remove duplicate files
import hashlib
import os
import optparse
import sys
...
def rm_dup(path, exps):
    ...
    if not os.path.isdir(path):  # make sure the given directory exists
        ...
        if len(ignore_list) > 0:
            print('Done! Found {} duplicate files, deleted {}, and ignored {} on user\'s request...'.format(
                c+len(ignore_list), c, len(ignore_list)))
        else:
            print('Done! Found and deleted {} files...'.format(c))


if __name__ == '__main__':
    print('    **************  Data Deduplication using Checksum  **************')
    print('    *              The script works on the understanding that             *')
    print('    *                 if 2 files have the same md5checksum                *')
    print('    *                  they most likely have the same content         *')
    print('    *     Hence, one of them should be deleted to avoid data duplication.     *')
    print('    ***************************************************************************    ')
    parser = optparse.OptionParser(
        "usage: python %prog -p <target path> -e <file extensions to ignore separated by ->")
    parser.add_option("-p", dest="target_path",
                      type="string", help="provide target path")
    parser.add_option("-e", dest="ext2ignore", type="string",
